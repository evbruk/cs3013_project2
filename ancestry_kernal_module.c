...
asmlinkage long new_sys_cs3013_syscall2(unsigned short *target_pid, struct ancestry *response) {
	...
	for(i = 0; i < 100; i++)
	{
		localResponse.children[i] = -1;
		localResponse.siblings[i] = -1;	
		if(i < 10)
			localResponse.ancestors[i] = -1;	
	}
	...
	while(parentPointer != parentPointer->real_parent) 
	{
		...
		if(parentPointer->pid > 0) 
		{
			if(ancesterCount < 10)
			{
				localResponse.ancestors[ancesterCount] = parentPointer->pid;
				ancesterCount++;			
			}
	...
	list_for_each_entry(working_task, &(currentTask->sibling), sibling)
	{
		...
		{
			localResponse.siblings[sibling_count] = working_task->pid;
		}else{
			break;		
		}
		sibling_count++;
	...
	printk(KERN_INFO "Copying to user space...\n");
	copy_to_user(response, &localResponse, sizeof(struct ancestry));
	//copy_to_user
	printk(KERN_INFO "Copied! Returning... \n");
	return 0;
}
